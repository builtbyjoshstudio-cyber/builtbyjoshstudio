...
- **Branch `kinetic-retheme` is FLIP-READY.** Conversion + dark-zodiac = the commits topping out at `678383d` (legal-dark); HANDOFF refreshes sit on top, unpushed (run `git rev-list --count main..HEAD` for the exact count ≈23, `git log --oneline` for HEAD). Tracked tree clean; only untracked `tools/_*.py` sweep scripts remain. ENTIRE SITE CONVERTED, **zodiac DARK everywhere**. **ONLY the live flip remains — await Josh's explicit go.** Backup tag `pre-kinetic-backup` = `39dc1e9` (= `main` tip) = instant rollback. **NEVER push without Josh's explicit go; one live flip = Phase 4.**

**✅ DONE — Phase-1 gate SIGNED OFF (fonts=self-host; zodiac was Light → **later flipped to DARK**, see the dark-zodiac iteration below). Foundation + homepage + 3 proofs (committed `664c5c7`/`09e2b91`):**
...
**✅ DONE — ENTIRE SITE CONVERTED (Phases 1–3 complete; per-archetype commits, all verified theme-only + `diff --check` clean, EOL preserved):**
- **Self-host fonts** (`bc32aa8`): tokens.css `@import`→`@font-face`; **18 woff2 (latin+latin-ext) tracked in `css/fonts/`**; `tools/_kinetic_selfhost_fonts.py`.
- **Central tokens.css fixes** (`17867cf`): per-archetype CTA **press-down hover** (overrides old inline orange-flip) + **`--tynkr-orange:var(--accent-2)`** alias (was undefined→ink; now orange — **changes live rendering** at flip).
- **Archetypes (hardened sweep `_kinetic_hex.py` / inverting `_kinetic_hex_zodiac.py`, theme-only):** products 8 (`474a5e3`, Tynkr **blue**) · blogs 35 (`34c1b1c`, **orange** editorial + blue CTA) · misc 7 about/privacy/terms/refunds/resources/free + **legal (dark→Light invert)** (`baab81a`) · hubs 3 blog.html/products-index/**collections-index(invert)** (`352290a`) · fiction 14 books+writing+characters, **Stage-2 wiring intact** (`21313bb`) · **collections 38 hand-swept Light-invert** (`a0f1564` — preserves view_item/FAQ/related/Etsy edits) · shared CSS 3 (`06e8372`, **gallery lightbox kept**) · free-card hard-border (`76ffc0a`).
- **Phase-3 cleanups:** deleted 214 redundant Google-font `<link>`s (`06e8d1d`, self-host now complete) · stripped 129 inline `backdrop-filter` glass decls (`9466510`) · regenerated `images/og/og-home.webp` as a Kinetic card (`2682b1c`).
- **VERIFIED:** census **ALL RETIRE rows 0** (9 old hexes + 4 old fonts; `backdrop-filter`=17 are intentional tokens-`none`/gallery-lightbox keeps). **Theme-only proof** `main` vs branch BYTE-IDENTICAL across 18 invariants (JSON-LD 432 · @type 3913 · price 82 · Offer 86 · FAQ 127 · Person 157 · Lemon-Squeezy 90 · etsy 428 · meta 1422 · canonical 108 · J.S.Warden 126 · Kansas 444 · AI-crafted 5).

**🌑 DARK-ZODIAC ITERATION (Josh changed his mind during review — zodiac is now DARK everywhere, NOT Light):**
- **Homepage** (`d0a9fa3`): re-darkened the 5 zodiac sections (`#builtbyjosh/#western-realms/#chinese-zodiac/#chinese-realms/#landscapes`) to match the dark BBJ hero — scoped `--bbj-*` override (bg `#14130e`, surface `#1d1b15`, cream text, border `#2f2c24`) + hardcoded `#b8aed0`→`#c9c6bd` + gold/purple glows→orange. **Light Tynkr zone / dark zodiac zone.**
- **tokens.css `[data-theme="dark"]` token block** (`e07932f`): flips `--canvas/surface/ink/muted/line/line-2/shadow/body-read` to the warm-dark palette. **All 40 collection pages** (`e07932f`) + **`legal/index.html`** (`678383d`) are tagged `html[data-theme="dark"]` → the nav, body, `--bbj-*` aliases, AND every `data-glass="cosmic"` component (cards/sidebar/chips) re-theme to dark **at once**. Flipped the art-tile + cta-band light gradient (`#f8f5ee/#f1eee6`→`#1d1b15/#14130e`) and the `collections/index` hub hero. Art **pops** on dark.
- **Verified** aries / aquarius-realms / collections-index hub / legal (dark nav+body `#14130e`, cards `#1d1b15`, cream text, orange accent, art pops, legal text readable); **0 residual light backgrounds** across 40; theme-only still **100%** (`main` vs branch invariants unchanged). **PREVIEW CACHE GOTCHA:** the browser caches `/css/tokens.css` (no query) so a collection page shows LIGHT until tokens.css reloads — bust via injecting a fresh `<link href="/css/tokens.css?bust=N">`; the live site serves fresh.
...
- `python tools/_kinetic_selfhost_fonts.py [--write]` — fetch 18 woff2 (latin+latin-ext) + inject `@font-face` into tokens.css. Run (`bc32aa8`).
- `python tools/_kinetic_striplinks.py <files…> [--write]` — delete per-page Google Fonts `<link>` lines. Run (`06e8d1d`).
- `python tools/_kinetic_deglass.py <files…> [--write]` — strip inline `backdrop-filter:…blur…` decls; keeps tokens-`none` + gallery lightbox + `@supports` tests. Run (`9466510`).
- `python tools/_kinetic_og.py` — Pillow 1200×630 Kinetic OG card → `images/og/og-home.webp` (needs Bricolage/Hanken TTFs in `tools/_fonts/`). Run (`2682b1c`).
- `python tools/_kinetic_census.py` → writes `tools/_kinetic_census.md`. **RETIRE rows all 0** now (except the intentional `backdrop-filter` keeps).

**▶ ONLY THE LIVE FLIP REMAINS (Phase 4 — await Josh's explicit go):**
- Review `git diff main...kinetic-retheme --stat` (**131 files · +6,691/−7,646** · 107 HTML + 4 CSS + 18 woff2 + 1 webp + HANDOFF) → on **explicit go**: merge `kinetic-retheme`→`main` + push (GitHub Pages deploys live ~1 min) → verify live cache-busted across archetypes. **`pre-kinetic-backup` (`39dc1e9`) = instant rollback.**
- **NOTE the `--tynkr-orange` alias changes live rendering** (the 8 product "Read the full breakdown" links go ink→orange) — the only intentional visible-content change beyond pure colour/font.
- **Non-blocking leftovers:** logo recolor (`images/logo/*` bitmaps bake navy/gold — Josh/external) · `overlayed-echoes-preview.html` left **intentionally bespoke** (standalone parchment reader, no tokens.css link — Josh's call) · og-home.webp is a *designed* Kinetic card, not a literal screenshot (no HTML→image-file in this env; swap if preferred) · stale comment in `mobile-nav.css` (cosmetic).

**PREVIEW METHOD (used this session — no wedge):** preview-MCP `static-site` config (`.claude/launch.json` → `python -m http.server 8080 --directory <repo>`) via `preview_start` → `preview_eval(window.location.assign('/<page>?v=N'))` → `preview_screenshot` / `preview_inspect` / `preview_console_logs` / `preview_network`. The preview-MCP screenshot path does NOT wedge (unlike the old Claude-in-Chrome CDP path). Server idles out between turns — just `preview_start` again. CSS cache-bust (the `<link>` has no query): `fetch('/css/x.css?x='+n)`.
