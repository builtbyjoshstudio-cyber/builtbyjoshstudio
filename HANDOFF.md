# ✅ 2026-08-19 (later) — SEO + GEO AUDIT + P0 batch LIVE (main `5fd8e1e` · tynkr `2e2f8ec`; pushed on "make it so")

**Audit:** workspace `handoffs/SEO-GEO-AUDIT-2026-08-19.md` — 178 pages parsed from code (head/meta/canonicals, headings, link graph, images, scripts, JSON-LD completeness + visible-price cross-check, template similarity, cannibalization pairs, robots/sitemap/llms) + Lighthouse 12 mobile lab on 6 live pages + live HTTP checks. Scorecard: infra A- · CWV **C** (lab mobile LCP fails on all 6: home 5.2 s, zodiac 5.5 s, blog 4.5 s, product 2.9 s but CLS 0.27, tools roast page **10 s** from ~1.1 MB of eager JPEG network cards) · titles/descriptions/social B- (66/131 titles >60, 75 descriptions >160, 17 pages without og:image, 38 zodiac og 720×900) · schema/entities B · content architecture B- (Chinese animals share 82% of text; blog↔tool title twins; /free/ vs /products/ titles one word apart = likely the "Google chose different canonical" page) · GEO B (robots posture good, llms.txt covers 54/130, no llms-full, brand casing split 303/251) · authority D. Roadmap P0–P3 in the doc; P1 = tools card JPEGs→webp+lazy+dims, LCP preload/fetchpriority + width/height sweep (383 imgs) + product-page CLS, 26 long titles, zodiac per-item copy, Person/publisher `@id` anchoring, /free/ retitle, tabletop cross-links.

**P0 shipped (tag `backup-pre-seo-p0` main + tynkr):** (1) **5 blog sidebars showed a stale `FROM $24.99`** (never-launched single-platform price) while schema said 22.99 — `blog/ultimate-budget-workbook`, `sinking-funds-explained`, `zero-based-budget-excel`, `first-time-homebuyer-mistakes`, `home-buying-mortgage-workbook` → `$22.99` (a real visible/schema mismatch and a plausible contributor to the 6 MC feed rejects via the website-crawl source); (2) Product `sku` on the 44 nodes that lacked one — 8 product pages (paid node) + 22 blog Product nodes + 2 planner cross-sells = Merchant-feed ids (`tynkr-*`), Chinese hub 12 per-animal nodes = `BBJ-CS-<ANIMAL>`; $0 Lite nodes deliberately left without sku; collections keep their existing `BBJ-WS/WR/CS-*` skus (they do NOT equal the feed's `bbj-*` ids — known inconsistency, harmless, unify later by changing page skus, never feed ids); (3) og:image (+ twitter:image; full OG/Twitter blocks on blog.html and the collections hub) on the 16 main pages that had none (12 posts reuse their product image 1600×1600; hubs/legal use `images/og/og-home.webp`), and og:image + Twitter card on tynkr privacy/terms. Script `tools/_seo_p0_batch.py` (untracked scratch). lastmod synced (3 posts). Validation: 516/516 + 11/11 JSON-LD, og files exist, no mixed EOLs; edge-verified 8/8 needles live; IndexNow runs fired on both repos.

# 🔎 2026-08-19 — GSC review + "crawled/discovered – not indexed" FIX BATCH (main `0f03a06` · tools `48a9580`) — **LIVE** (pushed on Josh's "make it so" 2026-08-19 ~15:11Z: main `f222efe..b0de6a9`, tools `49131c2..48a9580`; edge-verified cache-busted 12/12 needles; IndexNow CI runs 32268509745 / 32268515845 submitted 130 + 37 URLs, HTTP 200). **Validate-fix schedule given to Josh:** NOW → validFrom + returns/shipping (Merchant/Product-snippet issues) + "Alternate page with proper canonical tag" (redirect is live) + Request-indexing the 13 URLs (8 crawled-not-indexed + 5 weak-linked, ~10/day); **~Sep 2** → Validate "Crawled – currently not indexed" (after URL Inspection shows the retitled posts re-crawled); Sep 1 zodiac re-check; Sep 15 index.html-duplicate check.

**Review (full write-up: workspace `handoffs/GSC-REVIEW-2026-08-19.md`, from Josh's Performance + Coverage + Coverage-Drilldown exports dated 2026-08-19).** ⚠️ **The GSC property is a DOMAIN property — every number blends main + tools + tynkr** (Pages sheet: 85 main / 42 tools / 6 tynkr; sitemaps 130+37+7). Headline: impressions at a record (2,832/wk) but **58% are the tools site at avg pos 56.8** (calculator head terms on pages 4–8) → clicks flat (54 in 3 months, CTR 0.2%); main site alone 7,846 impr / 49 clicks / pos 15.3, sections holding (products 12.1 · blog 12.8 · collections 19.2). Money page unchanged (creator-content-os 22 clicks, pos 11.4). **Zodiac watch:** western signs weighted pos **19.6** (16.1–23.9), "wall art" queries 454 impr / 0 clicks @27 — top of page 2, not yet low-20s. Merchant listings surfacing (71 impr @ pos 1.0). Indexing: 53→124 by Jul 24 then flat; not-indexed 77 = 44 Discovered + **8 Crawled-not-indexed (validation FAILED)** + 19 alternate-canonical + 3 redirect + 2 noindex + 1 "Google chose different canonical". 58 sitemap URLs had zero impressions (whole `/writing/` section, 18 zodiac pages, 9 blogs, /free/, /planners/ + 2 planners, 5 tools). **10 legacy `tools/<x>/index.html` duplicates still indexed** holding 1,956 impr (19% of tools) despite correct canonicals everywhere.

**Drill-down (the 8 crawled-not-indexed URLs):** budget-workbook blog, brine-calculator blog, notion-business-os, notion-finance-os, what-lite-actually-means, sinking-funds-explained, how-i-cook-rice-finger-method, overlayed-echoes-preview. Measured: NOT thin (1.6k–9k words), NOT text-duplicates (≈0 shingle overlap with twins), NOT under-linked (5–11 inbound). Two were **title-twins** of an indexed sibling (budget blog vs product page; brine blog vs tools page); most were indexed, earned impressions with ~0 clicks, then culled; last crawls Apr 30–Jul 5 — and **sitemap lastmod was stale everywhere** (41/132 entries still at the bulk date 2026-05-22).

**Fix batch shipped (tags `backup-pre-gsc-crawled-fix` both repos):**
1. **De-twinned titles** — `blog/ultimate-budget-workbook.html` title → "A Budget Spreadsheet That Tracks Your Entire Financial Life (Excel & Google Sheets)" (= its H1/og/headline; distinct from the product page), description + twitter:description + Article schema description rewritten, dateModified 2026-08-19. `blog/brine-calculator.html` title/og/twitter → "Building the Brine Calculator: Why Wet & Dry Brining Math Is Worth Getting Right" (mirrors the indexed "Building the Universal Recipe Scaler"), descriptions rewritten, dateModified 2026-08-19; H1/headline unchanged.
2. **Related Reading cards (12)** from indexed posts into the never-crawled posts: dough-hydration + pan-swap → yeast-converter; recipe-scaler + batch-broths + meat-thawing → cost-per-serving; zero-based-budget + 50-30-20 + ultimate-budget-workbook → free-budget-spreadsheet; how-to-launch + complete-notion-os → track-etsy-gumroad; free-notion-templates + what-lite → free-tabletop-rpg-tools. Same `related-post-card` markup; `tools/_gsc_crawled_fix.py` (untracked scratch).
3. **`noindex,follow` on BOTH chapter-preview pages** (overlayed-echoes-preview, ebonspire-chronicles-preview — reader deep links, not SEO pages; Ebonspire done for consistency) + both removed from sitemap (130 URLs now).
4. **`/index.html` → `/` self-redirect** `<script>` inserted right after `<meta charset>` on every page (main 133 — the no-email redirect stub skipped — + tools 37). GitHub Pages can't 301; Google honours JS redirects; verified locally (`/products/index.html?x=1` → `/products/?x=1`, `/index.html` → `/`). Preserves query + hash; guarded to http(s) only.
5. **NEW TRACKED TOOL `tools/sitemap_lastmod_sync.py`** — lastmod = date of the page's last *content* change from git history (fingerprint = title + meta description + body minus nav/footer/script/style/comments, so chrome sweeps/JSON-LD/redirect snippets don't bump; uncommitted content change → today; never moves backwards). Applied: main 108/130 entries changed, tools 23/37 (`--root C:\Users\jotra\tools`). **STANDING RULE: run `python tools/sitemap_lastmod_sync.py --apply` (and `--root C:\Users\jotra\tools` for tools) before committing any content edit; commit the sitemap with it.**
Validation: JSON-LD 516/516 + 71/71, no mixed EOLs, sitemaps well-formed, headless render of a new card. Not done (deliberate): canonicalising/noindexing the 5 Notion-OS guides or the generic finance/cooking posts — they're fine content; the fix there is lastmod truth + Request Indexing + time. A first attempt at the commit accidentally staged the untracked scratch files (.netlify, tools/_fonts, audit txt) — caught and redone before anything left the machine (`0f03a06` is clean: 134 M + 1 A).

**JOSH AFTER PUSH (dashboard):** URL Inspection → Request indexing on the 8 crawled-not-indexed + the 5 weak-linked never-crawled posts (~10/day); Validate "Alternate page with proper canonical tag" (19) + the 3 July validations; re-validate "Crawled – currently not indexed" ~1 week after deploy; export the 1 "Duplicate, Google chose different canonical" URL (still unknown); ~Sep 15 check Pages→filter "index.html" for the duplicate impressions trending to zero; ~Sep 1 zodiac re-check (baseline now: western signs 19.6, "wall art" queries 27.0).

# 📕 2026-08-15 (later) — Cookbook PAPERBACK edition surfaced (main `d615cdb` · tools `49131c2`) — **LIVE** (both arcs pushed on Josh's "push it" 2026-08-15 evening: main `24b04bc..09e80ef`, tools `895bd36..49131c2`; edge-verified cache-busted 11/11 needles HTTP 200 on both hosts). **IndexNow CI proof:** the first REAL submissions from the Actions workflow fired — main run 31924118351 submitted **114 URLs**, tools run 31924120268 submitted **21 URLs**, both accepted (HTTP 200) — the "not yet exercised" caveat from 2026-07-20 is closed.

The ASIN flag from the YouTube arc is RESOLVED: Josh sent the Amazon screenshots — **`B0HBL7HZYN` is a paperback edition** of Nước Mắm ($7.99, 80 pp, 5×0.19×8 in, ISBN-13 979-8188988173, published July 25, 2026, sold/shipped by Amazon), **`B0HBCXTLWD` is the Kindle** ($3.99, 57 pp, July 24). Both verified live by fetching the /dp/ pages (paperback title carries the ISBN; Kindle title says "Kindle edition"). Site now carries the paperback everywhere the Kindle edition lives: **cookbook page** — hero eyebrow "Ebook & Paperback", third secondary CTA "Or get the paperback — $7.99 →", buy-note reworded, spec row "Paperback · $7.99 on Amazon · 80 pages", Book schema gained `workExample` (Paperback / isbn / numberOfPages 80 / datePublished 2026-07-25 / Amazon Offer $7.99, **no return/shipping policy on the Amazon offer** — standing rule) while the top-level Book stays the EBook with its 3 offers; **books.html** — formats line + "Get the paperback — $7.99 →" book-link (between Kindle and the YouTube line) + the same `workExample`; **llms.txt** (main + tools) paperback lines. Pattern mirrors OE/Ebonspire `workExample` editions. Not touched (deliberate): checkout-config (Amazon isn't LS; Kindle isn't in config either), the 15 cooking-blog sidebar cards ("$3.99, EPUB + PDF or Kindle" stays true), meta descriptions ("$3.99 ebook" still true), title. JSON-LD re-parsed (cookbook 2/2, books 8/8), CRLF/UTF-8 verified, headless PNGs of the buy card + books card. Tag `backup-pre-cookbook-paperback` both repos. STUDIO-WIKI (both copies) + memory updated. **After push: edge-verify the cookbook page + books.html cache-busted; consider adding a "paperback" line to the tools kitchen-page gear card ("Get the Cookbook — $3.99") only if Josh wants it.**

# ▶️ 2026-08-15 — "Josh & Jordan's Kitchen" YouTube channel added site-wide (main `8b2bbbe` · tools `86f4329`) — **LIVE** (pushed same day with the paperback arc, see the block above)

Task brief: `Downloads\youtube-website-update.md` — the cooking channel relaunched/rebranded from **Josh Cooks** (legacy `youtube.com/@joshcooksfood`, `youtube.com/user/jotran18`) to **Josh & Jordan's Kitchen**, canonical `https://www.youtube.com/@joshandjordanskitchen`, tagline "Real Home Cooking & Baking" (Josh = savory / Vietnamese classics, Jordan = the oven). **Audit first (all 3 site repos + workspaces): ZERO legacy handles anywhere; every existing channel link was the FICTION channel `@TalesofInkShadowsStudio` (136 on main, 53 on tools — Organization `sameAs` on every page + about.html Connect list/prose + llms.txt + tools kitchen-page gear card + tools hub Network column); 13 studio briefing videos on `resources/` (lite-yt facade, `?rel=0` is functional, not tracking) + the OE trailer nocookie embed; no `?si=` params anywhere.** So the spec's "replace legacy URLs" was a no-op and its "every YouTube link → cooking channel" check was read as "every *cooking-channel* link" — the fiction channel and video links were deliberately left alone (flagged to Josh).
- **Shipped (main, 138 files +281/−34):** footer `<li>YouTube</li>` right after Substack on **131 pages** (all pages with `footer-links`; the redirect stub + 2 chapter-preview pages have no footer list); channel appended after Tales in **every Organization `sameAs`** (127 blocks; multi-line + inline forms) **plus the 5 zodiac generators + `tools/identity_cleanup.py`** so regeneration keeps it; about.html Connect-list card + "Beyond the Studio" blurb (verbatim one-line description, links `/nuoc-mam-cookbook.html`); cookbook page: author `Person.sameAs`, "From the same kitchen" channel paragraph + third `series-btn` "Watch on YouTube →" (+ new `.series p a{color:#fff;underline}` rule — global `a{color:var(--accent)}` made the inline link INVISIBLE on the accent-brown band, caught by headless render); books.html: author `Person.sameAs` + a `book-link` line "Watch the kitchen on YouTube — Josh & Jordan's Kitchen →" (first draft as a classless inline `<a>` rendered default-blue — books.html has no global `a` rule; converted to the card's book-link pattern); llms.txt Beyond-the-studio bullet + companion clause on the cookbook line. **Tools (37 files +40/−0):** `sameAs` on all 37 pages + Network column `<li>Josh &amp; Jordan's Kitchen</li>` after Ink & Shadows on the 3 hubs. tynkr-site: nothing (no socials/sameAs — ties to the main Org via `@id`). Little Acre: nothing (deliberate brand separation).
- **Method:** `tools/_youtube_channel_sweep.py` (untracked; `--repo main|tools [--apply]`, dry-run asserts: 0/1 footer match per file, sameAs any==ml+il, JSON-LD block count + parse non-regression, CRLF-drift assert; each file's own EOL reused — LF-authored tabletop post + .py generators stayed LF). Hand edits via Edit tool (CRLF + UTF-8 preserved, verified byte-level). Tags `backup-pre-youtube-channel` both repos.
- **Acceptance (all pass):** (1) `grep -ri "joshcooksfood\|jotran18"` → empty in all 3 trees; (2) new URL 273× main / 40× tools, always bare, 0 `?si=`, exactly two channel handles exist, canonical URL HTTP 200; (3) 131/131 footers ordered Substack › YouTube › Contact; (4) JSON-LD 516/516 + 71/71 parse, tag balance OK, only intended files/lines touched. Headless-Chrome PNG proof: about bottom, cookbook band, books card, blog footer, tools hub footer (scratchpad).
- **⚠️ Flags for Josh:** the brief's cookbook ASIN **B0HBL7HZYN ≠ live Kindle B0HBCXTLWD** — NOT changed, needs confirmation → *RESOLVED same day: it's the paperback edition (see the block above).* Existing footer Etsy/Substack links on index.html lack `rel="noopener"` (pre-existing; new links all carry it). Optional follow-ups: kitchen tool pages' net-band gear card (needs a 600px `*-link.jpg`), cooking-blog sidebar mention, fiction channel in the visible footer. STUDIO-WIKI (both copies) + memory updated. **After push: edge-verify a footer + about + cookbook page cache-busted; IndexNow will auto-ping the 131+37 changed pages.**

# 📋 2026-07-26 — GSC returns/shipping schema rec FIXED (`e52da2a`, LIVE) + MC alerts decoded

Two Google emails, both expected: (1) GSC "Missing hasMerchantReturnPolicy / shippingDetails in offers" — NON-critical rec. **Fixed: all 96 first-party offers** now carry `MerchantReturnPolicy: MerchantReturnNotPermitted` (matches refunds.html all-sales-final) + `OfferShippingDetails` $0/0-day (instant digital). **The 14 Amazon-seller offers deliberately lack them** (declaring OUR policy on Amazon's offers = false). Discovery: zodiac collections + blogs ALREADY had these from their generators — the gap was products/free-lite/cookbook offers. JSON round-trip sweep with deep-equality gate; edge-verified live; IndexNow pinged 15 URLs. Tag `backup-pre-returns-shipping`. GSC "Validate fix" after a few days. (2) MC "active items 63→50" (Jul 24-25 overnight) = feed-migration churn (crawl items superseded by feed items re-entering review + the 6 rejected rows) — NO action; recheck count midweek, expect climb past 60. **Feed status: MC fetch caught the stale 8-row CDN copy 3 min post-deploy (max-age=600 race); after re-fetch, 41/47 ingested — 6 rejects unexamined; get the item-level error list from MC → PRODUCTS SOURCE 1 processing report if Josh surfaces it.**

# 🛒 2026-07-24 (feed) — Merchant feed expanded 8→47 products (`6cbda90`, LIVE)

`tynkr-feed.tsv` (live-served at site root — MC fetches it as a product source) went 8→**47**: 8 existing rows kept byte-identical + 38 zodiac bundles (`bbj-<slug>`, brand Built By Josh Studio, category 500044, is_bundle yes) + cookbook (`bbj-nuoc-mam-cookbook`, category 784), all sourced from page JSON-LD, validated (unique ids, `X.XX USD`, links+images exist). **EXCLUDED on purpose:** $0 lite SKUs (not allowed) and Amazon-sold books/planners (landing page isn't the purchase point → disapproval risk). **JOSH ACTION:** in MC → Product sources, if scheduled-fetch of the URL → "Fetch now"; if manual upload → re-upload or repoint to the URL. New digital items may sit "pending" a couple days; occasional per-item disapprovals on digital goods are normal, not breakage. Note: MC feed TSV reads back via text-mode python as LF (universal newlines) — check bytes, not text, when verifying CRLF.

# 🔗 2026-07-24 (final) — index.html link normalization LIVE (`4b0b0ed`)

GSC "Alternate page with proper canonical tag" (11 URLs, validation-failed) = INFORMATIONAL, not breakage — canonicals were working; validation "fails" while internal links keep minting `/index.html` duplicates. Fix: **621 internal hrefs normalized across 130 files** — relative (`../index.html`→`../`), root (`/index.html`→`/`), subdir (`money/index.html`→`money/`), and **22 ABSOLUTE forms (`https://builtbyjoshstudio.com/index.html#…`) which were the actual discovery vector** (tools' internal links were already clean — its GSC entries came from the main site's absolute links). Anchors preserved; external URLs untouched; full link-integrity check (134+32 files) + 516 JSON-LD blocks re-validated; per-file EOL profile preserved (the tabletop post is LF-authored — an all-CRLF assert is WRONG on this repo now). IndexNow auto-pinged 132 URLs HTTP 200. Tag `backup-pre-index-normalize` both repos (tools needed zero changes). **GSC: wait ~1-2 wks after deploy, THEN click Validate on the alternate-page issue; stragglers from old external links are harmless.**

# 📚 2026-07-24 (later) — Cookbook KINDLE LIVE + cross-linked site-wide (main `a1df96d` · tools `ca4b7df`, both LIVE)
