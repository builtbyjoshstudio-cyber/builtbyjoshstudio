...
- **Branch `kinetic-retheme` is FLIP-READY — 22 commits ahead of `main`.** Conversion + dark-zodiac tops out at `678383d` (legal-dark); the HANDOFF refresh sits on top, unpushed (`git log --oneline -1` shows it as HEAD; first-steps will read `[ahead 22]`). Working tree clean. ENTIRE SITE CONVERTED, **zodiac DARK everywhere**. **ONLY the live flip remains — await Josh's explicit go.** Backup tag `pre-kinetic-backup` = `39dc1e9` (main tip) = instant rollback. **NEVER push without Josh's explicit go; one live flip = Phase 4.**
