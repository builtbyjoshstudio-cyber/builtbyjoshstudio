...
- **⚠️ Flags for Josh:** the brief's cookbook ASIN **B0HBL7HZYN ≠ live Kindle B0HBCXTLWD** — NOT changed, needs confirmation. Existing footer Etsy/Substack links on index.html lack `rel="noopener"` (pre-existing; new links all carry it). Optional follow-ups: kitchen tool pages' net-band gear card (needs a 600px `*-link.jpg`), cooking-blog sidebar mention, fiction channel in the visible footer. STUDIO-WIKI (both copies) + memory updated. **After push: edge-verify a footer + about + cookbook page cache-busted; IndexNow will auto-ping the 131+37 changed pages.**
