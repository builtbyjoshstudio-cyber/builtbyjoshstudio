"""Generate per-sign Western Realms collection HTML pages from manifest.json.

Usage:
    python tools/build_realm_pages.py [SIGN ...]      # generate specific signs
    python tools/build_realm_pages.py                 # generate all 12

Pulls design data from images/zodiac/realms/manifest.json (built by
build_realm_thumbs.py). Writes collections/<sign>-zodiac-realms.html.
"""
import json
import sys
...
SIGN_META = {
    'Aries':       {'symbol': '♈', 'element': 'Fire',  'dates': 'March 21 - April 19',     'slug': 'aries'},
    'Taurus':      {'symbol': '♉', 'element': 'Earth', 'dates': 'April 20 - May 20',        'slug': 'taurus'},
    'Gemini':      {'symbol': '♊', 'element': 'Air',   'dates': 'May 21 - June 20',         'slug': 'gemini'},
    'Cancer':      {'symbol': '♋', 'element': 'Water', 'dates': 'June 21 - July 22',        'slug': 'cancer'},
    'Leo':         {'symbol': '♌', 'element': 'Fire',  'dates': 'July 23 - August 22',      'slug': 'leo'},
    'Virgo':       {'symbol': '♍', 'element': 'Earth', 'dates': 'August 23 - September 22', 'slug': 'virgo'},
    'Libra':       {'symbol': '♎', 'element': 'Air',   'dates': 'September 23 - October 22','slug': 'libra'},
    'Scorpio':     {'symbol': '♏', 'element': 'Water', 'dates': 'October 23 - November 21', 'slug': 'scorpio'},
    'Sagittarius': {'symbol': '♐', 'element': 'Fire',  'dates': 'November 22 - December 21','slug': 'sagittarius'},
    'Capricorn':   {'symbol': '♑', 'element': 'Earth', 'dates': 'December 22 - January 19', 'slug': 'capricorn'},
    'Aquarius':    {'symbol': '♒', 'element': 'Air',   'dates': 'January 20 - February 18', 'slug': 'aquarius'},
    'Pisces':      {'symbol': '♓', 'element': 'Water', 'dates': 'February 19 - March 20',   'slug': 'pisces'},
}


def build_design_card(d, sign, title):
    return f'''
          <div class="design-card">
            <img src="../{d["webp_path"]}" alt="{title} variant {d["variant"]} -- {sign} Zodiac Realm Art Print" loading="lazy" width="720" height="900">
...
def build_realm_block(title, designs, sign):
    cards = ''.join(build_design_card(d, sign, title) for d in designs)
    return f'''
        <div class="realm-block">
          <h3 class="realm-title">{title}</h3>
...
def build_page(sign, manifest):
    ...
    hub_thumb = sign_data['hub_thumb']

    # Group designs by realm title
    by_title = {}
    for d in designs:
        by_title.setdefault(d['title'], []).append(d)
    titles_ordered = sorted(by_title.keys())

    # Sort variants within each title by variant number
    for t in titles_ordered:
        by_title[t].sort(key=lambda d: int(d['variant']))

    realm_blocks_html = '\n'.join(build_realm_block(t, by_title[t], sign) for t in titles_ordered)
    ...
    for i, d in enumerate(designs, start=1):
        item_list_items.append(
            ...
            '        "@type": "ListItem",\n'
            f'        "position": {i},\n'
            f'        "name": "{d["title"]} (Variant {d["variant"]}) -- {sign} Zodiac Realm Art Print",\n'
            f'        "image": "https://builtbyjoshstudio.com/{d["webp_path"]}"\n'
            '      }'
        )
    item_list_json = ',\n'.join(item_list_items)

    titles_csv = ', '.join(titles_ordered)

    # The template uses double curly braces around CSS rules + JS, which the
    # f-string emits as single braces. Other {...} are real substitutions.
...
<head>
  ...
  {{ "@context": "https://schema.org", "@type": "BreadcrumbList", "itemListElement": [
    ...
    {{ "@type": "ListItem", "position": 3, "name": "{sign} Zodiac Realms", "item": "https://builtbyjoshstudio.com/collections/{slug}-zodiac-realms.html" }}
  ] }}
  </script>
  ...
  {{
    ...
    "mainEntity": {{
      "@type": "ItemList",
      "name": "{sign} Zodiac Realm Designs",
...
<body>
  ...
  <header class="collection-hero">
    <div class="breadcrumb"><a href="../index.html">Home</a><span class="sep">/</span><a href="index.html">Collections</a><span class="sep">/</span><span>{sign} Zodiac Realms</span></div>
    <div class="hero-eyebrow">{meta['symbol']} {sign} · {meta['element']} · {meta['dates']} · 4 Realms · 8 Designs</div>
    <h1 class="collection-title">{sign} Zodiac Realm Art Prints — <span class="italic-accent">The Full {sign} Bundle</span></h1>
    <p class="collection-tagline">All 8 hybrid cosmos-landscape designs for {sign} across 4 mythic realms. Two variants per realm, all rendered at print-ready resolution.</p>
    <div class="hero-image"><img src="../{hub_thumb}" alt="{sign} Zodiac Realm Art Print -- hero design" /></div>
    <div class="hero-meta-chips"><span class="chip"><strong>8 Designs</strong></span><span class="chip">4 Realms</span><span class="chip">Up to 6000×9000 px</span><span class="chip">Instant Download</span></div>
  </header>

  <div class="collection-main">
    <main class="main-column">
      <section>
        <h2>The {sign} Realms Bundle — 8 Designs Across 4 Realms</h2>
        <p>The {sign} realms bundle includes all 8 hybrid cosmos-landscape designs for the {sign} zodiac sign — two variants of each of the 4 {sign} realm titles. Every design is delivered at print-ready high resolution (up to 6000×9000 px) and is part of the broader Western Zodiac Realms series.</p>
        <p>The realms aren't portraits of the sign — they're <strong>environments</strong>, the worlds {sign} would inhabit. Each realm is its own atmosphere, designed to be hung as a piece in its own right or collected as a complete {sign} set.</p>
      </section>

      <section>
        <h2>All 8 {sign} Realm Designs</h2>
{realm_blocks_html}
      </section>
    </main>
    ...
    <aside class="sticky-sidebar">
      ...
      <div class="sidebar-price-note">The full 8-design {sign} realms bundle launches on Lemon Squeezy. Pricing to be set at launch.</div>
      <div class="sidebar-included-label">What's Included</div>
      <ul class="sidebar-included">
        <li>All 8 {sign} realm designs</li>
        <li>4 realm titles × 2 variants each</li>
  ...
  <section class="faq-section">
    ...
    <div class="faq-list">
      <div class="faq-item"><h3>What is a Zodiac Realm?</h3><p>A Zodiac Realm is a hybrid cosmos-landscape rendering of a zodiac sign — not a portrait of the sign, but the mythic environment the sign would inhabit. Each Western sign has 4 unique realm titles, with 2 variants each, totaling 8 designs per sign.</p></div>
...
def main():
    ...
    signs_to_build = sys.argv[1:] if len(sys.argv) > 1 else list(SIGN_META.keys())
    for sign in signs_to_build:
        if sign not in SIGN_META:
            print(f'SKIP: unknown sign {sign}')
            continue
        out_path = OUT_DIR / f'{SIGN_META[sign]["slug"]}-zodiac-realms.html'
